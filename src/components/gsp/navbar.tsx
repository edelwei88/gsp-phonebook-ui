...
export function Navbar({ className }: { className?: string }) {
  ...
  return (
    <header className={`flex mb-6 justify-between ${className}`}>
      ...
      <div className='flex items-center space-x-4'>
        <button
          className='cursor-pointer text-blue-600 px-4 py-2 rounded-[15px] hover:bg-blue-50 dark:hover:bg-davysgray'
          onClick={openHelpModal}>
          <QuestionMark />
        </button>
        <button
          className='text-blue-600 px-4 py-2 rounded-[15px] hover:bg-blue-50 dark:hover:bg-davysgray cursor-pointer'
          onClick={handleClick}>
          {theme === 'light' ? (
